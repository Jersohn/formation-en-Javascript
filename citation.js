...
let newCitation = document.getElementById("citation");
let newAuteur = document.getElementById("auteur");
let derniereCitation = 0;
let nbr = 0;

function nombreAleatoire(nombre) {
	return Math.floor(Math.random() * Math.floor(nombre));
}

function changeCitationHandler() {
	do {
		nbr = nombreAleatoire(citations.length);
	} while (nbr == derniereCitation);
	newCitation.textContent = citations[nbr][0];
	newAuteur.textContent = citations[nbr][1];
	derniereCitation = nbr;
}
nouveau.addEventListener("click", changeCitationHandler);
